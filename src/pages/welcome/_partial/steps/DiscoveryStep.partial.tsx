...
const DiscoveryStep = () => {
	...
	const sourcesList = onboardingData?.meta?.discovery_sources || SURVEY_OPTIONS;
		...
		<>
			...
			<div className='no-scrollbar max-h-[300px] space-y-2 overflow-y-auto pr-1'>
				{sourcesList.map((option) => {
					const isSelected = selectedSurvey === option.value;
					return (
						<motion.button
							whileTap={{ scale: 0.98 }}
							key={option.value}
							onClick={() =>
								dispatch({
									type: 'SET_FIELD',
									payload: { selectedSurvey: option.value },
								})
							}
							className={`flex w-full items-center justify-between rounded-xl border p-3.5 text-left font-semibold transition-all ${
								isSelected
									? 'border-primary-500 bg-primary-400/5 text-primary-600 ring-primary-500/20 dark:text-primary-400 ring-1'
