...
const OnboardingShellInner = () => {
		...
		if (onboardingData && !initialized) {
			...
			dispatch({
				...
				payload: {
					...
					// A user who already has a workspace - resuming onboarding,
					// or invited into someone else's - has this step behind them.
					// Carrying the id forward is what lets the plan step start
					// checkout, which is workspace scoped.
					...(workspace
						? {
	...
	const currentOrbitIcons = useMemo(() => {
		...
		// Nothing connected yet just means the user has not got there — keep the
		// panel populated with the apps they are about to see rather than empty.
		if (step === 5)
			return connectedApps.length > 0
				? connectedApps.map((a) => a.toLowerCase())
				: ['slack', 'github', 'gmail', 'google drive'];
		if (step === 6)
			return connectedApps.length > 0
				? connectedApps.map((a) => a.toLowerCase())
	...
	if (isLoading) {
		return (
			<div className='from-primary-50/80 flex min-h-screen items-center justify-center bg-gradient-to-br via-slate-50/90 to-rose-50/80 dark:from-zinc-950 dark:via-zinc-900 dark:to-zinc-950'>
				<div className='flex flex-col items-center gap-4'>
					<div className='from-primary-400 to-primary-400 text-primary-950 shadow-primary-500/25 flex h-16 w-16 animate-pulse items-center justify-center rounded-[2rem] bg-gradient-to-tr shadow-lg'>
	...
	return (
		<main className='from-primary-50/80 relative flex min-h-screen w-full flex-col justify-between overflow-x-hidden bg-gradient-to-br via-slate-50/90 to-rose-50/80 text-slate-950 transition-colors duration-300 dark:from-zinc-950 dark:via-zinc-900 dark:to-zinc-950 dark:text-zinc-50'>
				...
				<div className='mx-auto flex max-w-7xl items-center justify-between'>
					...
					{step > 1 && (
						<button
							...
							className='text-xs font-black text-slate-400 transition-colors hover:text-slate-600 dark:hover:text-zinc-200'>
							Skip Onboarding
						</button>
					)}
				</div>
			...
			{/* Main Wizard Card */}
			<section className='relative z-10 flex flex-1 items-center justify-center px-0 py-0 sm:px-4 sm:py-8 md:py-16'>
				<motion.div
					layout
					transition={{ type: 'spring', stiffness: 220, damping: 26 }}
					style={{ maxWidth: isDualColumn ? '1024px' : '480px' }}
					className='relative w-full overflow-hidden border-slate-200/80 bg-white/95 shadow-2xl backdrop-blur-xl max-sm:flex max-sm:min-h-screen max-sm:flex-col max-sm:rounded-none max-sm:border-none max-sm:bg-white max-sm:shadow-none sm:rounded-[2rem] sm:border dark:border-zinc-800/80 dark:bg-zinc-900/95 dark:max-sm:bg-zinc-900'>
					<div
						className={`grid ${isDualColumn ? 'lg:grid-cols-2' : 'grid-cols-1'} max-sm:flex max-sm:flex-1 max-sm:flex-col`}>
						{/* Left: Form */}
						<div className='flex min-h-[520px] flex-col justify-between p-6 max-sm:min-h-0 max-sm:flex-1 max-sm:px-5 max-sm:py-6 md:p-10'>
							<div>
			...
			<footer className='relative z-10 hidden w-full py-6 text-center text-[10px] font-bold text-slate-400 sm:block dark:text-zinc-600'>
