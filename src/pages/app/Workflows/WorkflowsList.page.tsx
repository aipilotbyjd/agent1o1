...
import { useStartRun } from '@/api/modules/runs';
...
const WorkflowsListPage = () => {
	...
	const publishWorkflowMutation = usePublishWorkflow(currentWorkspaceId);
	const startRunMutation = useStartRun(currentWorkspaceId);
	...
	const moveWorkflowsMutation = useMoveWorkflowsToFolder(currentWorkspaceId);
	...
	const workflows = useMemo<IWorkflow[]>(() => {
		...
		return apiWorkflowsResponse.map((w): IWorkflow => {
			return {
				...
				// "Active" means published — the API has no separate on/off flag.
				status: w.is_published ? 'active' : 'inactive',
				// The list endpoint doesn't join the latest run; History is where
				// run times live.
				lastRun: '—',
				folderId: w.folder_id || null,
				apps: getAppNames(w.nodes),
				hasUnpublishedChanges: w.has_unpublished_changes,
				tags: (w.tags ?? []).map((t) => ({ id: t.id, name: t.name, color: t.color })),
	...
	const draftWorkflowCount = useMemo(
	...
	const [, setDraggedWorkflowId] = useState<string | null>(null);
	...
	const filteredWorkflows = useMemo(() => {
		return workflows.filter((w) => {
			if (activeTab === 'drafts' && w.status !== 'inactive') return false;
			if (searchQuery) {
	...
	/**
	 * Deleting a folder does not delete what is in it — the FK is
	 * `nullOnDelete`, so its workflows fall back to root level.
	 */
	const handleDeleteFolder = async () => {
		if (!editingFolder) return;
		const count = (folderGrouped[editingFolder.id] ?? []).length;
		const confirmed = await confirm({
			...
			message: (
				<>
					...
					?{' '}
					{count > 0
						? `The ${count} workflow${count === 1 ? '' : 's'} inside will move to root level, not be deleted.`
						: 'The folder is empty.'}
				</>
			),
		});
		...
		try {
			const name = editingFolder.name;
			await deleteFolderMutation.mutateAsync(editingFolder.id);
			setEditingFolder(null);
			triggerToast(`Deleted folder "${name}"`, 'info');
		} catch {
			// Error is surfaced by the mutation hook
		}
	};
	...
	/**
	 * The API has no activate/deactivate switch: a workflow becomes "active"
	 * by publishing a version, and there is no way back to draft. So this
	 * publishes, and is a no-op on an already-published workflow with nothing
	 * new to ship.
	 */
	const handlePublish = async (id: string, e: React.MouseEvent) => {
		...
		if (!wf) return;
		if (wf.status === 'active' && !wf.hasUnpublishedChanges) {
			triggerToast('Nothing new to publish.', 'info');
			return;
		}
		try {
			await publishWorkflowMutation.mutateAsync({ workflowId: id });
	...
	const handleRunNow = async (id: string, e: React.MouseEvent) => {
		...
		const wf = workflows.find((w) => w.id === id);
		try {
			await startRunMutation.mutateAsync({ workflowId: id });
							...
							<form onSubmit={handleUpdateFolder} className='space-y-4'>
								...
								<div className='flex flex-col-reverse gap-2.5 pt-2 sm:flex-row sm:justify-end'>
									...
									<button
										...
										className='h-9.5 cursor-pointer rounded-xl border border-rose-200 px-5 text-xs font-bold text-rose-500 transition-all hover:bg-rose-50 disabled:pointer-events-none disabled:opacity-50 dark:border-rose-900/40 dark:hover:bg-rose-950/20'>
