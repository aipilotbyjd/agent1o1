...
import { useDismissOnboarding, useOnboardingState } from '@/api/modules/onboarding';
import { STATUS_BADGE_COLORS } from './_helper/dashboard.constants';

// ─── Helpers ────────────────────────────────────────────────────────────────
const formatRelativeTime = (value?: string | number | null) => {
	if (!value) return '—';
	// Accepts both epoch seconds/ms and the ISO timestamps the API returns.
	const ms =
		typeof value === 'string'
...
const DashboardPage = () => {
	...
	// The overview is window-scoped (default 7 days); run-stats supplies the
	// daily series the trend sparkline draws.
	const { data: overview, isLoading } = useDashboardOverview(currentWorkspaceId);
	const { data: runStats } = useRunStats(currentWorkspaceId);
	const runTotals = overview?.runs;

	// ─── Monitoring data ──────────────────────────────────────────────────────
	const runsToday = runTotals?.total ?? 0;
	const runningNow = runTotals?.in_flight ?? 0;
	const successRate = runTotals?.success_rate ?? 0;
	const avgMs = runTotals?.avg_duration_ms ?? 0;
	const recentRuns = overview?.recent_runs ?? [];
	// There is no dedicated "recent failures" feed — the failed runs already in
	// the recent list are the same records.
	const failures = recentRuns.filter((run) => run.status === 'failed');
	const failedCount = runTotals?.failed ?? failures.length;
	const byDay = runStats?.series ?? [];
	const trend = buildSpark(byDay.map((d) => d.total));
	const peakDay = Math.max(0, ...byDay.map((d) => d.total));
	const windowRunTotal = runTotals?.total ?? 0;
	const windowDays = overview?.window.days ?? 7;

	// RunResource carries workflow_id but not the workflow itself, so names are
	// resolved against the (already cached) workflow list.
	const { data: workflowList } = useWorkflows(currentWorkspaceId);
	const workflowNameById = useMemo(
		() => new Map((workflowList ?? []).map((w) => [w.id, w.name])),
		[workflowList],
	);
	const workflowName = (id: string | null) =>
		(id && workflowNameById.get(id)) || 'Workflow';
	...
	const handleOnboardingAction = (key: TOnboardingStepKey) => {
		switch (key) {
			case 'profile_picture':
				navigate(pages.settings.subPages.profile.to);
				break;
			case 'create_workspace':
				navigate(pages.onboarding.subPages.createWorkspace.to);
				break;
			case 'invite_team':
				navigate(pages.onboarding.subPages.inviteTeam.to);
				break;
			case 'choose_plan':
				navigate(pages.settings.subPages.plan.to);
				break;
			case 'role_selection':
			case 'discovery_survey':
				navigate(pages.onboarding.to);
				break;
		}
	};
						...
						<motion.div
							...
							<div className='flex flex-wrap items-start justify-between gap-4'>
								<div>
									...
									<h2 className='mt-1 text-sm font-black text-slate-950 dark:text-white'>
										{completedStepCount} of {onboarding.steps.length} milestones
										complete
									</h2>
								</div>
								<button
							...
							<div className='mt-3 h-1.5 overflow-hidden rounded-full bg-primary-100/50 dark:bg-primary-900/20'>
								<div
									...
									style={{
										width: `${Math.min(100, onboarding.percent)}%`,
									}}
								/>
							</div>
										...
										<button
											...
											<div className='flex min-w-0 flex-1 items-center gap-3.5'>
												...
												<div className='min-w-0 flex-1'>
													<span
														className={`block truncate text-xs font-black ${item.completed ? 'text-slate-500 dark:text-zinc-400' : 'text-slate-805 dark:text-zinc-100'}`}>
														{item.label}
													</span>
										...
										<div
											...
											<div className='flex shrink-0 items-center gap-3'>
												<span className='hidden text-[10px] font-semibold text-text-muted sm:block'>
													{formatRelativeTime(f.finished_at)}
												</span>
												<button
													onClick={() =>
														navigate(
							...
							{isLoading && recentRuns.length === 0 ? (
								...
								<div className='divide-y divide-border-main'>
									{recentRuns.slice(0, 6).map((run) => {
										const badge =
											STATUS_BADGE_COLORS[run.status as TRunStatus] ??
					...
					<div className='space-y-6'>
						...
						<div className='overflow-hidden rounded-3xl border border-border-main bg-bg-card p-5 shadow-sm'>
							...
							<div className='mt-3 flex items-center justify-between border-t border-border-main pt-3 text-[11px] font-bold'>
								<span className='text-text-muted'>Last {windowDays} days</span>
								<span className='text-text-main'>
									{windowRunTotal.toLocaleString()} runs
								</span>
							</div>
						</div>
