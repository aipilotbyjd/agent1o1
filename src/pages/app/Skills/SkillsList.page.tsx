...
const SkillsListPage = () => {
	...
	const { data: skills, isLoading } = useAgentSkills(currentWorkspaceId);
	...
	// GET /workspaces/{ws}/skills takes no query filters, so search, category
	// and visibility are applied to the fetched list here.
	const skillList = useMemo(() => {
		const query = searchQuery.trim().toLowerCase();
		return (skills ?? []).filter((skill) => {
			if (selectedCategory !== 'All' && skill.category !== selectedCategory) return false;
			if (visibility !== 'All' && skill.is_shared !== (visibility === 'Shared')) return false;
	...
	}, [skills, searchQuery, selectedCategory, visibility]);
								...
								return (
									<motion.article
										...
										<div className='mt-4 flex flex-wrap gap-2'>
											...
											{/* SkillResource carries no reference/script counts —
											    version and tags are what the list payload has. */}
											<span className='inline-flex items-center gap-1 rounded-lg border border-slate-200/50 bg-slate-50/50 px-2 py-1 text-[10px] font-semibold text-slate-600 dark:border-zinc-800 dark:bg-zinc-950/50 dark:text-zinc-400'>
												<Code2 size={11} className='text-primary-500' />v
												{skill.version}
											</span>
