...
import { requiredConnectorsForGraph } from './_helper/templates.helper';
...
const TemplatesCatalogPage = () => {
	...
	// The template endpoints return the workspace's full catalog with no
	// query filters, so search and category are applied here.
	const matchesFilters = useMemo(() => {
		const query = searchQuery.trim().toLowerCase();
		return (item: { name: string; description: string | null; category: string | null }) => {
	...
	// Query lists
	const { data: allWorkflowTemplates, isLoading: isWfsLoading } =
		useWorkflowTemplates(activeWorkspaceId);
	const { data: allAgentTemplates, isLoading: isAgentsLoading } =
		useAgentTemplates(activeWorkspaceId);
	const { data: allCollections, isLoading: isCollsLoading } =
		useTemplateCollections(activeWorkspaceId);
	// Used to work out which connectors a template's graph needs.
	const { data: nodeCatalog } = useGlobalNodeCatalog();
	const connectorsFor = (graph: { nodes?: unknown[] } | null | undefined) =>
		requiredConnectorsForGraph(graph, nodeCatalog);

	const workflows = useMemo(
		() => (allWorkflowTemplates ?? []).filter(matchesFilters),
		[allWorkflowTemplates, matchesFilters],
	);
	const agents = useMemo(
		() => (allAgentTemplates ?? []).filter(matchesFilters),
		[allAgentTemplates, matchesFilters],
	);
	const collections = useMemo(
		() => (allCollections ?? []).filter(matchesFilters),
		[allCollections, matchesFilters],
	);
	...
	const useWfMutation = useUseWorkflowTemplate(activeWorkspaceId || '');
	const deployAgentMutation = useUseAgentTemplate(activeWorkspaceId || '');

	// Nothing in the API marks a template as featured — the most-used ones
	// stand in for the old `is_featured` flag.
	const byUsage = <T extends { usage_count: number }>(items: T[]) =>
		[...items].sort((a, b) => b.usage_count - a.usage_count).slice(0, 3);
	const featuredWorkflows = useMemo(() => byUsage(workflows), [workflows]);
	const featuredAgents = useMemo(() => byUsage(agents), [agents]);
	...
	const handleDeployCollection = async (coll: any) => {
		...
		const progressItems = (coll.items ?? []).map((item: any) => {
			...
			const name = isAgent
				? agents.find((a) => a.id === item.templatable_id)?.name || 'Agent'
				: workflows.find((w) => w.id === item.templatable_id)?.name || 'Workflow';
			return {
				name,
				type: isAgent ? 'agent' : 'workflow',
				template_id: item.templatable_id,
		...
		for (let i = 0; i < progressItems.length; i++) {
			...
			try {
				if (item.type === 'workflow') {
					const res = await useWfMutation.mutateAsync({ id: item.templatable_id });
					const link = `/editor/edit-workflow/${activeWorkspaceId}/${res.id}`;
					setDeployProgress((prev) =>
						prev.map((p, idx) => (idx === i ? { ...p, status: 'success', link } : p)),
					);
				} else {
					const res = await deployAgentMutation.mutateAsync({ id: item.templatable_id });
					const link = `/agent/edit/${res.id}`;
					setDeployProgress((prev) =>
						prev.map((p, idx) => (idx === i ? { ...p, status: 'success', link } : p)),
					);
				}
											...
											<div
												...
												<div className='mt-auto flex items-center justify-between border-t border-zinc-100 pt-3 dark:border-zinc-900'>
													<div className='flex flex-wrap gap-1'>
														...
														{connectorsFor(wf.graph) &&
															connectorsFor(wf.graph).length > 2 && (
													...
													<button
														onClick={async (e) => {
															...
															try {
																...
																navigate(
																	`/editor/edit-workflow/${activeWorkspaceId}/${res.id}`,
																);
															} catch (err) {}
														}}
											...
											<div
												...
												<div className='mt-auto flex items-center justify-between border-t border-zinc-100 pt-3 dark:border-zinc-900'>
													<span className='truncate rounded-md border border-primary-200/30 bg-primary-50 px-2 py-0.5 text-[9px] font-bold text-primary-600 dark:bg-primary-950/20 dark:text-primary-400'>
														{agent.config?.model ?? 'Model not set'}
													</span>
													<button
														onClick={async (e) => {
															e.stopPropagation();
											...
											<div
												...
												<div className='mt-auto flex items-center justify-between border-t border-zinc-100 pt-3 dark:border-zinc-900'>
													<span className='rounded-full border border-amber-200/40 bg-amber-50 px-2 py-0.5 text-[9px] font-bold text-amber-700 dark:bg-amber-950/20 dark:text-amber-400'>
														{coll.items?.length ?? coll.item_count ?? 0} items
													</span>
													<button
														onClick={(e) => {
															e.stopPropagation();
									...
									<div
										...
										<div className='mt-auto flex items-center justify-between border-t border-slate-100/80 pt-3.5 dark:border-zinc-900/60'>
											<div className='flex flex-wrap gap-1'>
												...
												{(!connectorsFor(wf.graph) ||
													connectorsFor(wf.graph).length === 0) && (
												...
												{connectorsFor(wf.graph) &&
													connectorsFor(wf.graph).length > 1 && (
								...
								return (
									<div
										...
										<div className='mt-auto flex items-center justify-between border-t border-slate-100/80 pt-3.5 dark:border-zinc-900/60'>
											...
											<div className='flex items-center gap-1.5 text-[10px] font-bold text-slate-400 dark:text-zinc-500'>
												<Users className='h-3.5 w-3.5 text-slate-400/80 dark:text-zinc-650' />
												<span>{coll.item_count ?? coll.items?.length ?? 0} items</span>
											</div>
										</div>
									</div>
								);
											...
											<div className='flex flex-col gap-6'>
												...
												<div>
													...
													<div className='flex flex-wrap gap-2'>
														{connectorsFor(wfDetail.graph)?.map(
															(cred: any) => {
																const name =
																	typeof cred === 'string'
																		? cred
														...
														{(!connectorsFor(wfDetail.graph) ||
															connectorsFor(wfDetail.graph).length ===
																0) && (
									...
									<>
										{!agentDetail ? (
											...
											<div className='flex flex-col gap-6'>
												...
												<div className='grid grid-cols-2 gap-4'>
													...
													<div className='rounded-lg border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-800 dark:bg-zinc-900'>
														...
														<div className='text-xs font-bold text-zinc-800 dark:text-zinc-200'>
															{agentDetail.config?.model ?? 'Model not set'}
														</div>
													</div>
												</div>
												...
												{/* Pre-packaged tools */}
												{!!agentDetail.config?.tool_bindings?.length && (
														<div>
															<h3 className='mb-2 text-xs font-bold tracking-wider text-zinc-800 uppercase dark:text-zinc-200'>
																Available System Tools
															</h3>
															<div className='flex flex-col gap-2'>
																{agentDetail.config.tool_bindings.map(
																	(binding, idx) => {
																		const t = binding as {
																			name?: string;
																			description?: string;
												...
												<div>
													...
													<AgentChatMock
														...
														// No example transcripts ship with an agent
														// template on the current API.
																			...
																			<div className='flex-1'>
																				<div className='flex items-center gap-2.5'>
																					...
																					<span className='rounded border border-zinc-200/40 bg-zinc-50 px-1.5 py-0.5 text-[9px] font-bold text-zinc-400 uppercase dark:bg-zinc-950'>
																						{item.templatable_type === 'agent_template' ? 'agent' : 'workflow'}
																					</span>
																				</div>
