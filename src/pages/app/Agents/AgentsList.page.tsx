...
import AgentFormModal from './_partial/AgentFormModal.partial';
import TagPickerModal, { type ITagPickerTarget } from '../_partial/TagPickerModal.partial';
...
const AgentsListPage = () => {
	...
	const createAgentMutation = useCreateAgent(currentWorkspaceId);
	const updateAgentMutation = useUpdateAgent(currentWorkspaceId);

	const [tagTarget, setTagTarget] = useState<ITagPickerTarget | null>(null);
	const [formTarget, setFormTarget] = useState<TAgent | null>(null);
	...
	const openEdit = (id: string) => {
		setFormTarget((apiAgents ?? []).find((a) => a.id === id) ?? null);
		setIsFormOpen(true);
	};

	const handleFormSubmit = async (values: TCreateAgentDto) => {
		try {
			if (formTarget) {
				await updateAgentMutation.mutateAsync({ id: formTarget.id, body: values });
			} else {
	...
	const agents = useMemo<IAgentItem[]>(() => {
		...
		// AgentResource has no active flag, category column or chat counts —
		// the first tag stands in for the old category.
		return apiAgents.map((a) => ({
			...
			category: a.tags?.[0]?.name || 'Uncategorized',
			tags: (a.tags ?? []).map((t) => ({ id: t.id, name: t.name, color: t.color })),
	...
	const deleteFolder = useDeleteFolder(currentWorkspaceId);
	const moveAgents = useMoveAgentsToFolder(currentWorkspaceId);

	const handleCreateFolder = async () => {
		const name = window.prompt('Folder name');
		if (!name?.trim()) return;
		const folder = await createFolder.mutateAsync({ type: 'agent', name: name.trim() });
	...
	const handleRenameFolder = async (id: string, current: string) => {
		...
		await updateFolder.mutateAsync({ id, body: { name: name.trim() } });
	};
		...
		const confirmed = await confirm({
			...
			message: (
				<>
					...
					? The agents inside move to no folder — they are not deleted.
				</>
			),
		});
		if (!confirmed) return;
	...
	const handleMoveAgent = async (agentId: string, folderId: string | null) => {
		await moveAgents.mutateAsync({ agent_ids: [agentId], folder_id: folderId });
	};
	...
	const filteredAgents = useMemo(() => {
		return agents.filter((agent) => {
			...
			const matchesCategory =
				...
				agent.tags.some((tag) => tag.name === selectedCategory);
			const matchesFolder =
				selectedFolder === 'All' ||
				(selectedFolder === 'none' ? !agent.folderId : agent.folderId === selectedFolder);

			return matchesSearch && matchesCategory && matchesFolder;
		});
	}, [agents, searchQuery, selectedCategory, selectedFolder]);

	const handleDuplicate = async (id: string) => {
		try {
			// The copy lands in this list, so there is nowhere to navigate to.
			await duplicateAgentMutation.mutateAsync(id);
		} catch {
			// Error is surfaced by the mutation hook
		}
	};
		...
		<Container className='relative overflow-x-hidden overflow-y-auto bg-[#f8f9fc] !p-0 dark:bg-zinc-950'>
			...
			<div className='mx-auto flex w-full max-w-7xl flex-col space-y-8 p-4 sm:p-6 md:p-8'>
				...
				<div className='flex flex-col gap-5 xl:flex-row xl:items-center'>
					...
					{/* Folders */}
					<div className='no-scrollbar flex flex-wrap items-center gap-2 pb-1'>
						{[
							...
							<button
								...
								className={`h-9 shrink-0 cursor-pointer rounded-xl px-3.5 text-xs font-bold transition ${
									selectedFolder === f.id
						...
						<button
							...
							className='flex h-9 shrink-0 cursor-pointer items-center gap-1.5 rounded-xl border border-dashed border-slate-300 px-3.5 text-xs font-bold text-slate-400 transition hover:border-primary-400 hover:text-primary-500 dark:border-zinc-700'>
							<FolderPlus size={12} />
							New folder
						</button>
					</div>

					{/* Tag tabs — driven by the workspace's real tags */}
								...
								<motion.article
									...
									<div className='flex items-start justify-between gap-4'>
										...
										<div className='flex items-center gap-2'>
											<span className='flex h-8 items-center justify-center rounded-xl border border-primary-500/20 bg-primary-400/10 px-3.5 text-xs font-semibold text-primary-600 capitalize dark:bg-primary-400/5 dark:text-primary-400'>
												{agent.provider}
											</span>
									...
									<div className='mt-4 flex flex-wrap gap-2'>
										...
										<select
											aria-label={`Folder for ${agent.name}`}
											value={agent.folderId ?? ''}
											onChange={(e) =>
												handleMoveAgent(agent.id, e.target.value || null)
											}
											className='h-[26px] cursor-pointer rounded-lg border border-slate-200/50 bg-slate-50/50 px-2 text-[10px] font-semibold text-slate-600 outline-none dark:border-zinc-800 dark:bg-zinc-950/50 dark:text-zinc-400'>
										...
										<button
											...
											className='inline-flex cursor-pointer items-center gap-1 rounded-lg border border-dashed border-slate-300 px-2 py-1 text-[10px] font-semibold text-slate-400 transition hover:border-primary-400 hover:text-primary-500 dark:border-zinc-700'>
											<CategoryIcon size={11} />
											{agent.tags.length ? 'Edit tags' : 'Add tags'}
										</button>
									</div>
									...
									<div className='flex items-center justify-between gap-3'>
										...
										<div className='flex items-center gap-2'>
											{/* A run is a chat session, which lives on the agent
											    page — this opens it rather than starting one
											    blind from the list. */}
											<button
												onClick={() =>
													navigate(
			...
			<AgentFormModal
				...
				open={isFormOpen}
				target={formTarget}
				isPending={createAgentMutation.isPending || updateAgentMutation.isPending}
