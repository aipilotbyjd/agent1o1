...
import type { TCredentialFieldSchema, TCredentialType } from './_types/credentialType.type';
import {
	useCredentials,
	useCredential,
	useCreateCredential,
	...
	useCredentialTypes,
} from './_helper/connectors.helper';
...
const AppsListPage = () => {
	...
	const { data: credentialsData, refetch: refetchCredentials } =
		useCredentials(activeWorkspaceId);
	const {
	...
	const setDefaultCredentialMutation = useSetDefaultCredential(activeWorkspaceId);
	...
	const handleAuthorize = async () => {
		...
		try {
			...
			if (isOAuthCredentialType(credentialType) && credentialType) {
				const { authorize_url } = await connectOAuthMutation.mutateAsync({
					connectorId: credentialType.id,
					name: credentialName.trim() || `My ${selectedAppForAuth.name} Connection`,
				});
				// The provider lives outside the router — full document navigation.
		...
		<motion.div
			...
			<div className='mt-6 flex items-center justify-between gap-3 border-t border-slate-100 pt-4 dark:border-zinc-800/60'>
				...
				<div className='flex flex-wrap items-center justify-end gap-2'>
					...
					<button
						disabled={isDeleting || isSettingDefault || credential.is_default}
