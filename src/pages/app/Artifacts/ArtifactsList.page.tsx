...
const ArtifactsListPage = () => {
	...
	// The list endpoint pages internally, so it answers { artifacts, meta }.
	const artifactList = artifacts?.artifacts ?? [];
	const totalSize = artifactList.reduce((acc, a) => acc + a.size, 0);
	const agentCount = new Set(
		artifactList.map((a) => a.agent?.id).filter(Boolean),
	).size;
									...
									<motion.article
										...
										<div className='mt-5 text-left'>
											...
											<p className='mt-2 text-xs leading-relaxed font-semibold text-slate-400 dark:text-zinc-500'>
												{formatBytes(artifact.size)} · {artifact.agent?.name ?? 'Uploaded'}
											</p>
										</div>
