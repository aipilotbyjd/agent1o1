...
const SecretsPage = () => {
	...
	/** `GET .../secrets` takes no query params, so the search box filters the
	 *  fetched list here instead of round-tripping. */
	const { data: allSecrets = [], isLoading } = useSecrets(activeWorkspaceId);

	const secrets = useMemo(() => {
		const q = searchQuery.trim().toLowerCase();
		if (!q) return allSecrets;
	...
	const createVar = useCreateSecret(activeWorkspaceId);
	const updateVar = useUpdateSecret(activeWorkspaceId);
	const deleteVar = useDeleteSecret(activeWorkspaceId);
