...
import { useNodeRuns } from '@/api/modules/runs';
...
/**
 * The old backend served a flat `/executions/{id}/logs` feed. The new one has
 * no logs endpoint at all - the nearest thing is `/runs/{id}/node-runs`, one
 * row per node. They are folded into the same `{ level, message, timestamp }`
 * line shape here so the viewer below stays exactly as it was.
 */
const ExecutionLogsViewer = ({ ws, executionId }: ExecutionLogsViewerProps) => {
	const { data: nodeRuns = [], isLoading } = useNodeRuns(ws, executionId);
	...
	const logs = useMemo(
		() =>
			nodeRuns.map((node) => ({
				...
				message: node.error
					? `${node.key} (${node.type}): ${node.error}`
					: `${node.key} (${node.type}) - ${node.status}`,
				timestamp: node.finished_at ?? node.started_at ?? '',
			})),
		[nodeRuns],
	);
