...
const MagicLinkPage = () => {
	...
	// The API exposes no passwordless/magic-link endpoint (see
	// routes/api/internal/auth.php) — until it does, this page cannot send
	// anything, so it says so rather than pretending the mail went out.
	const formik = useFormik({
		...
		onSubmit: (_values, actions) => {
			actions.setStatus(
				'Magic-link sign-in is not enabled on this account yet. Use your password to sign in.',
			);
			setSent(false);
		},
	});
						...
						<AnimatePresence>
							{formik.status && (
								<motion.p
									...
									animate={{ opacity: 1, height: 'auto' }}
									exit={{ opacity: 0, height: 0 }}
									className='rounded-2xl bg-amber-50 px-4 py-3 text-xs font-bold text-amber-700'>
									{formik.status}
								</motion.p>
							)}
						</AnimatePresence>
