...
const SettingsFieldRow = ({
	...
	<div className='grid gap-4 border-b border-zinc-100/80 py-5 lg:grid-cols-[260px_1fr] lg:items-center dark:border-zinc-800/80'>
		<div className='flex items-start gap-4'>
			<div className='bg-primary-50 text-primary-500 dark:bg-primary-950/30 dark:text-primary-400 flex h-9 w-9 shrink-0 items-center justify-center rounded-xl'>
				<FieldIcon size={16} />
			</div>
			<div>
...
const WorkspacePage = () => {
	...
	const isDirty = useMemo(() => {
		...
		return workspaceName.trim() !== activeWorkspace.name || timezone !== DEFAULT_TIMEZONE;
	...
	return (
		<div className='mx-auto w-full max-w-[1180px] px-6 py-8 sm:px-10 lg:px-14'>
			...
			<form onSubmit={handleSubmit}>
				...
				<div className='mb-6 rounded-2xl border border-zinc-100 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950'>
					<div className='flex flex-wrap items-center gap-5'>
						<div className='bg-primary-100 text-primary-600 dark:bg-primary-400/15 dark:text-primary-300 flex h-20 w-20 items-center justify-center rounded-[22px] text-3xl font-black'>
							{getInitials(displayName)}
						</div>
						<div>
							<h2 className='flex items-center gap-2 text-lg font-black tracking-tight text-zinc-950 dark:text-zinc-50'>
								<span>{displayName}</span>
								<span className='bg-primary-50 text-primary-600 dark:bg-primary-950 dark:text-primary-400 rounded-md px-2 py-0.5 text-[10px] font-bold'>
									{workspaceRole}
								</span>
							</h2>
				...
				{/* Fields Card */}
				<div className='rounded-2xl border border-zinc-100 bg-white px-6 py-2 shadow-sm dark:border-zinc-800 dark:bg-zinc-950'>
					{/* Workspace Name */}
					<SettingsFieldRow
						icon={Building2}
			...
			{isOwner && (
				<div className='mt-8 rounded-2xl border border-zinc-100 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950'>
					<button
						type='button'
