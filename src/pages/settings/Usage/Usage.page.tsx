...
import { useCreditBalance, useCreditTransactions } from '../_shared/billing.helper';
import { useUsageSnapshots } from '../_shared/billing.helper';
...
const UsagePage = ({ hideHeader = false }: { hideHeader?: boolean }) => {
	...
	// The credit-usage report is windowed in days, not by explicit dates.
	const { data: snapshots } = useUsageSnapshots(activeWorkspaceId, { days: 30 });
	...
	const workflowCredits = txList
		.filter((t) => t.type === 'node_run')
		.reduce((s, t) => s + Math.abs(t.credits), 0);
	const aiCredits = txList
		.filter((t) => t.type === 'agent_step')
		.reduce((s, t) => s + Math.abs(t.credits), 0);
	const codeCredits = txList
		.filter((t) => t.type === 'eval_case' || t.type === 'session_evaluation')
		.reduce((s, t) => s + Math.abs(t.credits), 0);
