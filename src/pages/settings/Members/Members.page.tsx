...
} from '@/api/modules/workspace-members';
import { useWorkspaceContext } from '@/context/workspace';

import type { TWorkspaceMember } from '@/types/workspace-extras.type';
import type { TAssignableWorkspaceRole, TWorkspaceRole } from '@/types/workspace.type';
...
const MembersPage = () => {
	...
	const sendInvitationMutation = useInviteWorkspaceMember(workspaceId);
	const cancelInvitationMutation = useRevokeWorkspaceInvitation(workspaceId);

	const hasWorkspace = Boolean(workspaceId);
	const isResolvingWorkspace = isWorkspacesLoading && !workspaceId;

	// Mutators
	const handleRoleChange = async (userId: string, newRole: TAssignableWorkspaceRole) => {
		if (!hasWorkspace) return;
		try {
			await updateRoleMutation.mutateAsync({ userId, body: { role: newRole } });
		} catch {
	...
	const handleInviteMember = async (e: React.FormEvent) => {
		...
		try {
			await sendInvitationMutation.mutateAsync({
				email: inviteEmail,
				role: inviteRole as TAssignableWorkspaceRole,
			});
