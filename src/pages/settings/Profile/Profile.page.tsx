...
const ProfilePage = () => {
	...
	const formik = useFormik<TProfileForm>({
		...
		onSubmit: async (values, actions) => {
			...
			try {
				// PATCH /user takes `name` and nothing else — the API has no
				// email-change flow, so the email field is display-only.
				await updateProfile.mutateAsync({ name: nextName });
			} catch (error) {
				if (ApiError.is(error)) {
					const fieldErrors = error.fieldErrors();
					actions.setErrors({ firstName: fieldErrors.name });
				}
			}
		},
	});
	...
	const isProfileDirty = Boolean(userData && fullName !== userData.name);
