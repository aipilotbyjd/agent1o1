...
const ChannelRow = ({ channel }: { channel: TNotificationChannel }) => {
	...
	const handleSaveEdit = async () => {
		...
		if (editState.url.trim()) {
			if (!/^https?:\/\/.+/.test(editState.url.trim())) {
				errs.url = 'Must be a valid URL';
			} else {
				config = { url: editState.url.trim() };
			}
		}
		...
		const body: TUpdateNotificationChannelDto = { name: editState.label.trim() };
			...
			{editState ? (
				...
				<div className='flex flex-wrap items-center gap-4'>
					<div className='flex min-w-0 flex-1 items-center gap-3'>
						<ChannelBadge type={channel.type} />
						<div className='min-w-0'>
							...
							<div className='mt-0.5 text-xs font-medium text-zinc-400 dark:text-zinc-500'>
								{/* The endpoint never returns config back —
								    the target URL is write-only. */}
		...
		<form
			...
			<div className='mb-4 flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 dark:border-amber-700/40 dark:bg-amber-500/10'>
				...
				<p className='text-xs font-medium text-amber-700 dark:text-amber-400'>
					Webhook URLs are encrypted at rest and{' '}
